...
class Tiles: TestCase {
  override class var title: String { "Tiles" }
  let count = 10
  var fixtureCount = 0
  ...
  override func step() {
    ...
    row1.stringValue = String(format: "dynamic tree height = %d, min = %d", height, Int(minimumHeight))
    row2.stringValue = String(format: "create time = %6.2f ms, fixture count = %d", createTime, fixtureCount)
  ...
  let row1 = NSTextField(labelWithString: "dynamic tree height = %d, min = %d")
  let row2 = NSTextField(labelWithString: "create time = %6.2f ms, fixture count = %d")
  
  var _customView: NSView?
  override var customView: NSView? {
    if _customView == nil {
      let stackView = NSStackView(views: [row1, row2])
