...
class BodyTypes: TestCase {
  ...
  override var customView: NSView? {
    if _customView == nil {
      let dynamicButton = NSButton(title: "Dynamic", target: self, action: #selector(onDynamic))
      let staticButton = NSButton(title: "Static", target: self, action: #selector(onDynamic))
      let kinematicButton = NSButton(title: "Kinematic", target: self, action: #selector(onDynamic))
      let stackView = NSStackView(views: [dynamicButton, staticButton, kinematicButton])
  ...
  @objc func onDynamic(_ sender: NSButton) {
    platform.setType(b2BodyType.dynamicBody)
  }
  
  @objc func onStatic(_ sender: NSButton) {
    platform.setType(b2BodyType.staticBody)
  }

  @objc func onKinematic(_ sender: NSButton) {
