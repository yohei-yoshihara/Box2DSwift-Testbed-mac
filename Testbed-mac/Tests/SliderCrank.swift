...
class SliderCrank: TestCase {
  override class var title: String { "Slider Crank" }
  ...
  override var customView: NSView? {
    if _customView == nil {
      let friction = NSButton(title: "Friction", target: self, action: #selector(onFriction))
      let motor = NSButton(title: "Motor", target: self, action: #selector(onMotor))
      let stackView = NSStackView(views: [friction, motor])
