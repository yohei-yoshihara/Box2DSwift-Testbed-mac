...
class ApplyForce: TestCase {
  ...
  override var customView: NSView? {
    if _customView == nil {
      ...
      let upButton = NSButton(title: "Up", target: self, action: #selector(onUp))
      let rightButton = NSButton(title: "Right", target: self, action: #selector(onRight))
      let stackView = NSStackView(views: [leftButton, upButton, rightButton])
  ...
  @objc func onUp(_ sender: NSButton) {
