...
class OneSidedPlatform: TestCase, b2ContactListener {
  ...
  let label = createLabelField(labelWithString: "Character Linear Velocity:")
  let textField = createValueField(labelWithString: "0.0")
  
  override var customView: NSView? {
    if _customView == nil {
      let stackView = NSStackView(views: [label, textField])
