...
class Web: TestCase {
  override class var title: String { "Web" }
  var bodies = [b2Body?]()
  var joints = [b2Joint?]()
  ...
  override var customView: NSView? {
    if _customView == nil {
      let destroyBody = NSButton(title: "Destroy Body", target: self, action: #selector(onDestroyBody))
      let destroyJoint = NSButton(title: "Destroy Joint", target: self, action: #selector(onDestroyJoint))
      let stackView = NSStackView(views: [destroyBody, destroyJoint])
  ...
  @objc func onDestroyBody(_ sender: NSButton) {
    for i in 0 ..< 4 {
      if bodies[i] != nil {
  ...
  @objc func onDestroyJoint(_ sender: NSButton) {
    for i in 0 ..< 8 {
      if joints[i] != nil {
