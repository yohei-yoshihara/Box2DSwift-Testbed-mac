...
class VerticalStack: TestCase {
  override class var title: String { "Vertical Stack" }
  let columnCount = 5
  let rowCount = 16
  ...
  override var customView: NSView? {
    if _customView == nil {
      let shootButton = NSButton(title: "Shoot", target: self, action: #selector(onShoot))
      _customView = shootButton
    }
    return _customView
  }

  @objc func onShoot(_ sender: Any) {
    if bullet != nil {
