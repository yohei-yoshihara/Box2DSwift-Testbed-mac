...
class EdgeShapes: TestCase {
  override class var title: String { "Edge Shapes" }
  struct Const {
  ...
  var _customView: NSView? = nil
  let popupButton = NSPopUpButton(frame: .zero, pullsDown: false)

  override var customView: NSView? {
    if _customView == nil {

      popupButton.addItems(withTitles: ["1", "2", "3", "4", "5", "6"])
      ...
      let gridView = NSGridView(views: [
        [popupButton, dropButton],
        [deleteButton],
      ])
      _customView = gridView
    }
    return _customView
  }
  
  @objc func onDropStuff(_ sender: NSButton) {
    let index = popupButton.indexOfSelectedItem
    create(index)
  }
  
  @objc func onDeleteStuff(_ sender: NSButton) {
    destroyBody()
  }
}
