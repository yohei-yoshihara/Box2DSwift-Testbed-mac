...
class MotorJoint: TestCase {
  override class var title: String { "MotorJoint" }
  ...
  @objc func onStart(_ sender: NSButton) {
    go = !go
  }
...
//    m_textLine += 15;
  ...
  override var customView: NSView? {
    if _customView == nil {
      let button = NSButton(title: "Start", target: self, action: #selector(onStart))
      _customView = button
    }
    return _customView
  }
