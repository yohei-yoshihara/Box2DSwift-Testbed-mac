...
class TheoJansen: TestCase {
  ...
  override var customView: NSView? {
    if _customView == nil {
      let left = NSButton(title: "Left", target: self, action: #selector(onLeft))
      let brake = NSButton(title: "Brake", target: self, action: #selector(onBrake))
      let right = NSButton(title: "Right", target: self, action: #selector(onRight))
      let motor = NSButton(title: "Motor", target: self, action: #selector(onMotor))
      let stackView = NSStackView(views: [left, brake, right, motor])
  ...
  @objc func onMotor(_ sender: NSButton) {
