...
class PolyShapes: TestCase {
  override class var title: String { "Polygon Shapes" }
  struct Const {
  ...
  override var customView: NSView? {
    if _customView == nil {
      popupButton.addItems(withTitles: ["1 (filtered)", "2", "3", "4", "5", "6"])
      let dropStuffButton = NSButton(title: "Drop", target: self, action: #selector(onDropStuff))
      let dropStack = NSStackView(views: [popupButton, dropStuffButton])
      dropStack.orientation = .horizontal
      let modeChangeButton = NSButton(title: "Activate", target: self, action: #selector(onActivate))
      let deleteStuffButton = NSButton(title: "Delete", target: self, action: #selector(onDeleteStuff))
      
      let stackView = NSStackView(views: [dropStack, modeChangeButton, deleteStuffButton])
  ...
  @objc func onActivate(_ sender: Any) {
    var i = 0
