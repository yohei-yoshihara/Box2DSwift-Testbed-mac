...
class Revolute: TestCase {
  ...
  override var customView: NSView? {
    if _customView == nil {
      let limitButton = NSButton(title: "Limits", target: self, action: #selector(onLimits))
      let motorButton = NSButton(title: "Motor", target: self, action: #selector(onMotor))
      let stackView = NSStackView(views: [limitButton, motorButton])
  ...
  @objc func onLimits(_ sender: Any) {
