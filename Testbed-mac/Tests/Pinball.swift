...
class Pinball: TestCase {
  override class var title: String { "Pinball" }
  ...
  override var customView: NSView? {
    if _customView == nil {
      let label = NSTextField(labelWithString: "On/Off")
      let switchButton = NSSwitch(frame: .zero)
      switchButton.target = self
      switchButton.action = #selector(onSwitch)
      let stackView = NSStackView(views: [label, switchButton])
  ...
  @objc func onSwitch(_ sender: Any) {
    button = !button
  }
